...
# 这里导入数据集类：
# ViNT_Dataset 用于图像目标导航，
# LeLaN_Dataset 用于语言条件导航训练。
from vint_train.data.vint_dataset import ViNT_Dataset
from vint_train.data.lelan_dataset import LeLaN_Dataset

# 这里导入顶层训练循环。
# 本文件主要负责“组装系统”，真正的训练细节在这些 train_eval_loop_* 中。
...
def main(config):
    # 基本配置检查。
    # 即使 LeLaN 自己的 yaml 没显式写这两项，它们也会从 defaults.yaml 里继承。
    assert config["distance"]["min_dist_cat"] < config["distance"]["max_dist_cat"]
    assert config["action"]["min_dist_cat"] < config["action"]["max_dist_cat"]

    # 选择可见 GPU，并确定本次训练使用的主设备。
    ...
    cudnn.benchmark = True  # 输入尺寸基本固定时，这样通常更快
    transform = ([
    ...
    # 数据集构建
    # ---------------------------------------------------------------------
    # 遍历 yaml 里声明的所有数据集，分别构建 train/test。
    # LeLaN 之所以走单独的数据集类，是因为它的样本里除了图像，还包含语言提示和目标相对位姿。
    train_dataset = []
    test_dataloaders = {}
    ...
    # 把多个训练数据集合并成一个大训练集，再封装成 DataLoader。
    ...
    # 模型构建
    # ---------------------------------------------------------------------
    # 根据 model_type 创建对应的策略模型。
    # 每个分支的本质都是：把当前模型需要的编码器和预测头拼起来。
    ...
    # ---------------------------------------------------------------------
    # load_run 用来恢复主模型训练。
    # load_nomad 只在 lelan_col 中使用，用于加载一个预训练 NoMaD 作为避障监督。
    current_epoch = 0
    ...
    # 多卡训练时，用 DataParallel 包一层。
    if len(config["gpu_ids"]) > 1:
        model = nn.DataParallel(model, device_ids=config["gpu_ids"])
    model = model.to(device)

    if "load_run" in config:  # DataParallel 之后再恢复优化器和调度器
    ...
    # ---------------------------------------------------------------------
    # 这个文件到这里就完成“系统组装”了。
    # 后面会根据 model_type，把控制权交给对应的训练循环函数。
